...
from server.config.application.routers import setup_routes_v1
from ..settings import _make_fastapi_instance_kwargs, Config
...
class DevelopmentApplicationBuilder(BaseFastAPIApplicationBuilder):
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.app: FastAPI = FastAPI(**_make_fastapi_instance_kwargs(app_settings=self._config.application))  # type: ignore
        self.app.config = self._config  # type: ignore
        self._openapi_schema: Optional[Dict[str, Any]] = None

    def configure_openapi_schema(self) -> None:
        self._openapi_schema = get_openapi(
            title=self._config.application.project_name,
            version=self._config.application.version,
            description=self._config.application.description,
    ...
    def setup_middlewares(self):
        ...
        self.app.add_middleware(
            ...
            allow_origins=self._config.application.backend_cors_origins,
            allow_credentials=True,
            allow_methods=ALLOWED_METHODS,
            allow_headers=self._config.application.allowed_headers,
            expose_headers=["User-Agent", "Authorization"],
        )
        self.app.add_middleware(
            middleware_class=SessionMiddleware,
            secret_key=self._config.application.secret_key
        )

    @no_type_check
    def configure_routes(self):
        self.app.include_router(setup_routes_v1(self._config.application))
    ...
    def configure_application_state(self) -> None:
        ...
        injector.register(Settings, lambda: self._config)
